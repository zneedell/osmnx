"""Calculate graph edge capacities."""
...
def add_edge_lanes(G, hwy_lanes=None, fallback=None, precision=None, agg=np.nanmedian):
    """
    ...
    hwy_lanes : dict
        ...
        assigned the median preexisting lane number value of all edges of that highway
        type.
    fallback : numeric
    ...
    if "lanes" in edges.columns:
        ...
        # into a single value
        edges["lanes"] = edges["lanes"].apply(_collapse_multiple_lane_values, agg=agg)

        # create speed_kph by cleaning maxspeed strings and converting mph to
        # kph if necessary
        edges["lanes"] = edges["lanes"].astype(str).map(_clean_lanes).astype(float)
    else:
    ...
    if hwy_lanes is None:
        hwy_lanes_avg = pd.Series(dtype=float)
    else:
        hwy_lanes_avg = pd.Series(hwy_lanes).dropna()
    ...
    for hwy, group in edges.groupby("highway"):
        if hwy not in hwy_lanes_avg:
            hwy_lanes_avg.loc[hwy] = agg(group["lanes"].astype(np.float64))
    ...
    hwy_lanes_avg = hwy_lanes_avg.fillna(fallback).fillna(agg(hwy_lanes_avg))

    # for each edge missing speed data, assign it the imputed value for its
    # highway type
    lanes_numeric = (
        edges[["highway", "lanes"]].set_index("highway").iloc[:, 0].fillna(hwy_lanes_avg)
    )
    ...
    if pd.isnull(lanes_numeric).all():
        msg = (
...
def add_edge_capacities(G, precision=None):
    """
...
def _clean_lanes(lanes, agg=np.max):
    """
    ...
    clean_value : string
    """
    if isinstance(lanes, numbers.Number):
        return lanes
    # regex adapted from OSM wiki
    # pattern = "^([0-9][\\.,0-9]+?)(?:[ ]?(?:km/h|kmh|kph|mph|knots))?$"
    values = re.split(r"\|", lanes)  # creates a list even if it's a single value
    try:
        clean_values = []
        for value in values:
...
def _collapse_multiple_lane_values(value, agg):
    """
    ...
    try:
        ...
        values = [_clean_lanes(x) for x in value]
